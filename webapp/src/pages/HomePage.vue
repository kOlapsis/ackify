...
function handleLoginClick() {
  router.push({ name: 'auth-choice', query: { redirect: route.fullPath } })
}
  ...
  <div class="min-h-[calc(100vh-8rem)]">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8">
      ...
      <div v-else-if="docId && currentDocument" class="space-y-6">
        ...
        <!-- Login prompt banner (shown when an unauthenticated visitor lands on a shared link) -->
        <div
          v-if="!isAuthenticated"
          class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 sm:p-5"
        >
          <div class="flex flex-col sm:flex-row sm:items-center gap-4">
            <div class="flex items-start gap-3 flex-1">
              ...
              <div class="flex-1">
                <h3 class="font-semibold text-blue-900 dark:text-blue-200">{{ t('sign.loginPrompt.title') }}</h3>
                <p class="mt-1 text-sm text-blue-700 dark:text-blue-300">{{ t('sign.loginPrompt.description') }}</p>
              </div>
            </div>
            <button
              @click="handleLoginClick"
              class="trust-gradient text-white font-medium rounded-lg px-5 py-2.5 text-sm hover:opacity-90 transition-opacity whitespace-nowrap inline-flex items-center justify-center gap-2"
            >
              <LogIn :size="16" />
              {{ t('sign.loginPrompt.button') }}
            </button>
          </div>
        </div>
